...
(defun paradox-current-buffer ()
  ...
  (let ((paradox-buffer (get-buffer-create "*paradox*"))
    ...
    (save-excursion
	  ...
	  (insert "  " *paradox-program* " " additional-e-arguments " " tptp-file))
      (newline 2)
      (insert "Results:")
      (newline)
      (insert +report-separator+)
	  ...
	  (call-process *paradox-program* nil t t additional-e-arguments tptp-file))
      (setf buffer-read-only t))))
...
(defun vampire-current-buffer (additional-vampire-arguments)
  ...
  (let ((vampire-buffer (get-buffer-create "*vampire*"))
    ...
    (save-excursion
	  ...
	  (insert "  " *vampire-program* " " additional-e-arguments " < " tptp-file))
      (newline 2)
      (insert "Results:")
      (newline)
      (insert +report-separator+)
	  ...
	  (call-process *vampire-program* tptp-filet t additional-e-arguments))
      (setf buffer-read-only t))))
