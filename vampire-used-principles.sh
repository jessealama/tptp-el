...
if [ -z $proof ]; then
    echo "Usage: `basename $0` VAMPIRE-PROOF TPTP-BACKGROUND-THEORY";
    exit 1;
fi
...
if [ ! -f $proof ]; then
    echo "Error: the supplied vampire proof '$proof' is not a regular file";
    exit 1;
fi

if [ ! -r $proof ]; then
    echo "Error: the supplied vampire proof '$proof' is not readable";
    exit 1;
fi
